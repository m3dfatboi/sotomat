...
def create_tournament(body: TournamentCreate):
    ...
    with get_db() as conn:
        cur = conn.execute("INSERT INTO tournaments (name, prize_mode) VALUES (?,?)", (name, prize_mode))
        conn.commit()
...
def delete_tournament(tid: int):
    with get_db() as conn:
        if not conn.execute("SELECT id FROM tournaments WHERE id=?", (tid,)).fetchone():
...
def remove_tournament_player(tid: int, pid: int):
    with get_db() as conn:
        ...
        if not conn.execute("SELECT id FROM tournament_players WHERE id=? AND tournament_id=?", (pid, tid)).fetchone():
            raise HTTPException(404, "Игрок не найден")
...
def finish_tournament(tid: int, body: TournamentFinish):
    with get_db() as conn:
        ...
        for p in players_rows:
            ...
            rating_changes[name] = (delta, place)

            # Update player rating
            if delta != 0:
                conn.execute("UPDATE players SET rating=rating+? WHERE name=?", (delta, name))
        ...
        conn.execute(
            "UPDATE tournaments SET status='finished', winner_name=?, second_name=?, third_name=?, bracket_json=? WHERE id=?",
